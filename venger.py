...
def reduce_rows(a):
    a = deepcopy(a)
    row_mins = [min(row) for row in a] #минимум каждой строки
...
def find_zero_matching(mat):
    ...
    match_row = [-1] * n
    def dfs(i, seen):
        # dfs(поиск в глубину) для строки i
        # seen столбцы, которые уже пробовали в этой попытке
        for j in range(n): #проход по всем столбцам
            if mat[i][j] == 0 and not seen[j]:
                #not seen[j] — столбец j ЕЩЕ НЕ ПРОБОВАН в этой ветке поиска
                seen[j] = True
                if match_col[j] == -1 or dfs(match_col[j], seen):
                    # СЛУЧАЙ 1: match_col[j] == -1 — столбец j СВОБODЕН → занимаем
                    # СЛУЧАЙ 2: match_col[j] != -1 — столбец занят → РЕКУРСИЯ!
                    #           dfs(match_col[j], seen) спрашивает у ВЛАДЕЛЬЦА: "Можешь уйти?"
                    match_col[j] = i #строка i занимает столбец j
                    return True #нашли назначение
...
def hungarian_min(cost):
    ...
    if -1 in assignment:
        raise ValueError("Не удалось найти полное назначение только по нулям.")
